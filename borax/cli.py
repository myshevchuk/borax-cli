...
def cmd_summary(library_path: str):
    ...
    summary = history_tracker.library_summary(config.root, config.history_path, config.bib_path)
...
def cmd_scan(library_path: str):
    config = load_library_config(library_path)
    stats = tagging.scan_library(config.root, config.history_path, config.vocab, verbose=True)
...
def cmd_bibtex(library_path: str):
    ...
    print(f"Added {added} new entries to {config.bib_path}")
...
def main():
    parser = argparse.ArgumentParser(description="Borax (Book Organizer and Research Article arXiver)")
